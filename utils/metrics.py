...
def geodesic_distance(input, target, eps:float = 1e-7, reduction: str="mean"):
    # Input (N, ..., 3, 3)
    # Target (N, ..., 3, 3)
    ...
    if isinstance(input, np.ndarray):
        ...
        is_np = True
    R_diffs = input @ target.transpose(-2, -1)
...
def quaternion_to_matrix(quaternions: torch.Tensor) -> torch.Tensor:
    ...
    将单位四元数转换为3x3旋转矩阵。
    输入:
        quaternions: (..., 4) 张量，最后一维为 (w, x, y, z)
    输出:
        rotation_matrices: (..., 3, 3) 张量
    """
    assert quaternions.shape[-1] == 4, "输入最后一维必须是4 (w, x, y, z)"
    ...
    rotation_matrix = torch.stack([row0, row1, row2], dim=-2)
