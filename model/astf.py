...
from model.networks import StyleTransformer, MCRDiscriminator

class ASTF(nn.Module):
    def __init__(self, istrain, cfg, args):
        ...
        self.MCRdiscriminator = MCRDiscriminator(cfg, num_frame=200, dim_emb=args.dim_emb, num_heads=args.num_heads,
                            num_enc_blocks=args.num_disc_blocks)

        self.istrain=istrain
        if istrain:
            self.optimizer_MCR = torch.optim.Adam(
                        params=self.MCRdiscriminator.parameters(),
    ...
    def save_checkpoint(self, latest=False):
        ...
        checkpoint['discriminator'] = self.MCRdiscriminator.state_dict()
        checkpoint['optimizer_G'] = self.optimizer_G.state_dict()
        checkpoint['optimizer_D'] = self.optimizer_MCR.state_dict()
    ...
    def load_checkpoint(self):
        ...
        self.MCRdiscriminator.load_state_dict(checkpoint['discriminator'])
    ...
    def load_midtrain_checkpoint(self, load_path):
        checkpoint = torch.load(load_path, map_location='cuda:0')
        self.generator.load_state_dict(checkpoint['generator'])
        self.MCRdiscriminator.load_state_dict(checkpoint['discriminator'])
