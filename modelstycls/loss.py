...
def class_loss(args, model, cnt_clip, cnt_label, cnt_mask, criterion):
    _, outputs = model.generator(cnt_clip, cnt_mask)

    loss = criterion(outputs, cnt_label)

    _, predicted = torch.max(outputs.data, 1)
    total = cnt_label.size(0)
    correct = (predicted == cnt_label).sum().item()

    acc = 100 * correct / total

    loss_dict = OrderedDict([('crossloss', loss.item()),
                             ('acc', acc)
                            ])
